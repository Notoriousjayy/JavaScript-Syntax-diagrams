 ...
 * 
 * Detects drift between diagram factories (es2025Grammar.ts) and EBNF definitions (ebnfDefinitions.ts).
...
// Extract rule names from es2025Grammar.ts by parsing rules.set() calls
...
function main() {
  ...
  const grammarPath = resolve(srcDir, 'es2025Grammar.ts');
  ...
  try {
    ...
    console.log(`📊 Found ${diagramRules.size} diagram rules in es2025Grammar.ts`);
  } catch (err) {
    console.error(`❌ Failed to read es2025Grammar.ts: ${err.message}`);
    process.exit(1);
  }
